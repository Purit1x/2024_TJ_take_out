...
const filterCoupon = () => {
  ...
  // 筛选符合条件的优惠券
  availableCoupons.value[ModifyingMerchant.value] = coupons.value.filter(coupon => {
  ...
  if(CouponType.value===0){
    availableCoupons.value=availableCoupons.value.filter(coupon => coupon.couponType===0);
  }
};
