...
const renewTurnoverStat = async () => {
    try {
        ...
        if (ordersData === 0) {
            ...
            totalTurnoverWithinThisMonth.value = 0;
            const promise = ordersData.map(async orderItem => await getMerPrice(orderItem));
        ...
        if (ordersDayData === 0) {
            ...
            totalTurnoverWithinThisDay.value = 0;
            const promise = ordersDayData.map(async orderItem => await getMerPrice(orderItem));
            const fees = await Promise.all(promise);
            for (const fee of fees) {
    ...
    <div class="content">
            ...
            <el-form :model="currentMerchant" :rules="merchantRules" ref="refForm" label-width="150px" v-else>
                <div class="updateinfo">
                    ...
                    <el-form-item label="营业开始时间" prop="TimeforOpenBusiness">
                        <el-time-picker placeholder="选择日期" v-model="currentMerchant.TimeforOpenBusiness"
                            :picker-options="{ selectableRange: '00:00:00 - 23:59:59' }"></el-time-picker>
                    </el-form-item>
                    <el-form-item label="营业结束时间" prop="TimeforCloseBusiness">
                        <el-time-picker placeholder="选择日期" v-model="currentMerchant.TimeforCloseBusiness"
                            :picker-options="{ selectableRange: '00:00:00 - 23:59:59' }"></el-time-picker>
                    </el-form-item>
                </div>
        ...
        <el-form :model="currentMerchant" :rules="merchantRules" ref="refForm">
            <div class="recharge" v-if="isRecharge"> <!-- 充值 -->
                <div>充值金额</div>
                <el-form-item label="充值金额" prop="recharge"><input type="number" v-model="currentMerchant.recharge"
                        placeholder="请输入充值金额" @blur="validateField('recharge')" /></el-form-item>
                <button @click="SaveRecharge">充值</button>
                <button @click="leaveRechargeWindow">关闭</button>
            </div>
            <div class="withdraw" v-if="isWithdraw"> <!-- 提现 -->
                <div>提现金额</div>
                <el-form-item label="提现金额" prop="withdrawAmount"><input type="number"
                        v-model="currentMerchant.withdrawAmount" placeholder="请输入提现金额"
                        @blur="validateField('withdrawAmount')" /></el-form-item>
                <button @click="SaveWithdraw">提现</button>
                <button @click="leaveWithdrawWindow">关闭</button>
            </div>
            <div class="changewp" v-if="isChangeWP"> <!-- 修改支付密码 -->
                <div>支付密码</div>
                <el-form-item label="支付密码" prop="WalletPassword"><input type="password"
                        v-model="currentMerchant.WalletPassword" placeholder="请输入支付密码"
                        @blur="validateField('WalletPassword')" /></el-form-item>
                <div>确认支付密码</div>
                <el-form-item labal="确认支付密码" prop="reWalletPassword"><input type="password"
                        v-model="currentMerchant.reWalletPassword" placeholder="请再次确认支付密码"
                        @blur="validateField('reWalletPassword')" /></el-form-item>
